-- PSP graphics: Canvas / Shader stubs, SpriteBatch, Text.
--
-- The PSP exposes no render target, so Canvas draws straight to the screen.
...
function love.graphics.newShader(c)
...
-- ── SpriteBatch ──────────────────────────────────────────────────
-- Queued draws replayed one by one; there is no hardware batching to use.
function love.graphics.newSpriteBatch(img, max, usage)
    ...
    function sb:add(q, x, y, r, sx, sy, ox, oy)
        if type(q) == "table" and q.getViewport then
            ...
            table.insert(self._sprites, {x=q, y=x, r=y, sx=r, sy=sx, ox=sy, oy=ox})
        end
